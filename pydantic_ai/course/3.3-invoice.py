...
class Invoice(BaseModel):
    ...
    due_date: str = Field(
        ..., description="The date by which payment is expected to be made."
    )
    currency: str = Field(
    ...
    total_amount_due: float = Field(
        ..., description="The final amount due after including tax."
    )
...
# Run the agent
result = agent.run_sync(
    f"Can you extract the following information from the invoice? The raw data is {invoice_data}"
)
